#!/usr/bin/env python3
"""
One-click ROS mission runner for drone_planner_node + stanley_controller_node.

Flow:
1) Wait for planner services and pose stream
2) Arm planner
3) Set wp0 via /drone_planner/target_pos + /drone_planner/update_target
4) Wait until wp0 reached
5) Set wp1 and wait until wp1 reached
6) Save trajectory plot + GIF + JSON report under sets/plots

Prerequisites:
- roscore is running
- drone_planner and stanley_controller are running
  (e.g., roslaunch sets drone_planner.launch)
"""
...
# -- frame transform helpers (same convention as drone_planner_node) --
...
def transform_position(px, py, pz, frame_config):
    """Apply position transform ENU -> NED (planner frame)."""
...
def planner_to_mocap_position(px, py, pz, frame_config):
    """Inverse of transform_position for position-only mapping (NED -> ENU)."""
    scale = _parse_scale_xyz(frame_config.get("mocap_to_planner_scale_xyz", [1.0, 1.0, 1.0]))
    rz_deg = float(frame_config.get("mocap_to_planner_rz_deg", 0.0))
    mocap_frame = frame_config.get("mocap_frame", "ENU")
...
def planner_yaw_to_mocap_yaw(planner_yaw, frame_config):
    """Approximate inverse yaw mapping for ENU/NED."""
    rz_deg = float(frame_config.get("mocap_to_planner_rz_deg", 0.0))
    rad = math.radians(rz_deg)
    mocap_frame = frame_config.get("mocap_frame", "ENU")
...
# -- ROS test pose node (Mode B: internal simulation) --

class DroneRosTestPoseNode:
    """ROS test node for virtual pose publishing using UCT rollout states.
    ...
    def __init__(self, pose_topic, cmd_topic, frame_config, config_name, sim_rate_hz, mode="rollout"):
        ...
        self._dots.set_param(
            ...
            int(cfg.get("dots_decision_making_horizon", 60)),
            int(cfg.get("dots_dynamics_horizon", 20)),
        ...
        self._uct = get_uct2()
        sim_uct_N = int(rospy.get_param("/drone_planner/uct_N", cfg.get("uct_N", 500)))
        sim_uct_max_depth = int(rospy.get_param("/drone_planner/uct_max_depth", cfg.get("uct_max_depth", 12)))
        sim_uct_wct = float(rospy.get_param("/drone_planner/uct_wct", cfg.get("uct_wct", 1200.0)))
        sim_uct_c = float(rospy.get_param("/drone_planner/uct_c", cfg.get("uct_c", 3.0)))
        sim_uct_heuristic = rospy.get_param("/drone_planner/uct_heuristic_mode", cfg.get("uct_heuristic_mode", "shuffled"))
        sim_uct_tree_exploration = rospy.get_param("/drone_planner/uct_tree_exploration", cfg.get("uct_tree_exploration", "uct"))
        sim_uct_downsample = bool(rospy.get_param("/drone_planner/uct_downsample_traj_on", cfg.get("uct_downsample_traj_on", True)))
        self._uct.set_param(
            sim_uct_N, sim_uct_max_depth, sim_uct_wct, sim_uct_c,
            False, False, False, False, False,
            sim_uct_heuristic, sim_uct_tree_exploration, sim_uct_downsample, False,
        )
        sim_seed = int(rospy.get_param("/drone_planner/seed", cfg.get("seed", 0)))
        self._rng = RNG()
        self._rng.set_seed(sim_seed)
        self._mpc_steps = int(rospy.get_param("/drone_planner/uct_mpc_depth", cfg.get("uct_mpc_depth", 2)))
        ...
        rospy.loginfo("DroneRosTestPoseNode started: mode=%s pose=%s cmd=%s dt=%.3f seed=%d",
        ...
        try:
            if self._mode == "cmd_dynamics":
                ...
                if self._mdp.is_state_valid(nxt):
                    ...
                    rospy.logwarn_throttle(5.0, "DroneRosTestPoseNode state invalid; holding")
            else:
                if not self._segment:
                    ...
                    if getattr(res, "success", False):
                        ...
                        rospy.logwarn_throttle(5.0, "DroneRosTestPoseNode rollout planning failed")

                if self._segment:
                    self._state = np.array(self._segment.pop(0), dtype=np.float64)
        except Exception as e:
            rospy.logwarn_throttle(5.0, "DroneRosTestPoseNode step failed: %s", e)
...
class DroneTestRunner:
    def __init__(self, args):
        ...
        self.pose_topic = args.pose_topic or rospy.get_param("/drone_planner/pose_topic", "/pose")
        self.cmd_topic = args.cmd_topic or rospy.get_param("/stanley_controller/cmd_vel_topic", "/cmd_vel")
        self.traj_topic = args.traj_topic or rospy.get_param("/drone_planner/trajectory_topic", "/planner/trajectory")

        self.frame_config = {
            "mocap_frame": rospy.get_param("/drone_planner/mocap_frame", "ENU"),
            "mocap_to_planner_rz_deg": rospy.get_param("/drone_planner/mocap_to_planner_rz_deg", 0.0),
            "mocap_to_planner_scale_xyz": rospy.get_param("/drone_planner/mocap_to_planner_scale_xyz", [1.0, 1.0, 1.0]),
        }
        ...
        self.latest_traj_stamp = None
        self.traj_received_count = 0
        ...
        self.traj_sub = rospy.Subscriber(self.traj_topic, RosPath, self._traj_cb, queue_size=5)

        self._test_pose_node = None
        if self.args.use_ros_test_node:
            self._test_pose_node = DroneRosTestPoseNode(
        ...
        rospy.wait_for_service("/drone_planner/arm", timeout=15.0)
        rospy.wait_for_service("/drone_planner/update_target", timeout=15.0)
        self.arm_srv = rospy.ServiceProxy("/drone_planner/arm", SetBool)
        self.update_target_srv = rospy.ServiceProxy("/drone_planner/update_target", Trigger)

    def _cmd_cb(self, msg):
        with self._cmd_lock:
            self.latest_cmd[:] = [msg.linear.x, msg.linear.y, msg.linear.z, msg.angular.z]

    def _traj_cb(self, msg):
        with self._traj_lock:
            self.latest_traj_stamp = msg.header.stamp
            self.traj_received_count += 1
    ...
    def _pose_cb(self, msg):
        ...
        with self._traj_lock:
            traj_ok = self.latest_traj_stamp is not None
    ...
    def _set_target(self, wp):
        ...
        rospy.set_param("/drone_planner/target_pos", target)
        ...
        while not rospy.is_shutdown():
            ...
            if dt - last_log > 1.0 and dist is not None:
                if p_now is not None:
                    rospy.loginfo("[%s] dist=%.2fm pos=(%.2f,%.2f,%.2f), remaining %.1fs",
    ...
    def run(self):
        ...
        self.phase = "done"
        return {"wp0_dist": float(d0), "wp1_dist": float(d1)}
...
def load_obstacle_boxes(config_name="policy_convergence_drone"):
...
def save_plots(samples, wp0, wp1, out_prefix, config_name="policy_convergence_drone"):
    if not samples:
    ...
    yr = np.array([s["yr"] for s in samples], dtype=np.float64)
    traj_ok = np.array([s.get("traj_ok", False) for s in samples], dtype=bool)
    ...
    boxes = load_obstacle_boxes(config_name)

    png_path = out_prefix + ".png"
    gif_path = out_prefix + ".gif"

    # -- PNG: 2x2 subplot layout --
    ...
    ax.plot(t, vz, linewidth=0.9, label="cmd vz")
    ax.plot(t, yr, linewidth=0.7, alpha=0.6, label="cmd yaw_rate")
    ax.set_xlabel("time (s)")
    ax.set_ylabel("cmd_vel")
    ax.set_title("/cmd_vel history")
    ax.legend(fontsize=8)
    ...
    for bx, by, bz in boxes:
        x_min, x_max = min(bx[0], bx[1]), max(bx[0], bx[1])
        y_min, y_max = min(by[0], by[1]), max(by[0], by[1])
        z_min, z_max = min(bz[0], bz[1]), max(bz[0], bz[1])
    ...
    metrics["trajectory_msg_count"] = 0
    metrics["trajectory_msg_ratio"] = 0.0
    metrics["cmd_nonzero_ratio"] = 0.0
...
def main():
    ap = argparse.ArgumentParser(description="Drone Planner + Stanley Controller two-waypoint test")
    ap.add_argument("--wp0", type=parse_wp, default=[-20.0, 0.0, -120.0],
                    help="Waypoint 0 in planner frame (NED), format: x,y,z")
    ap.add_argument("--wp1", type=parse_wp, default=[20.0, 0.0, -120.0],
                    help="Waypoint 1 in planner frame (NED), format: x,y,z")
    ...
    ap.add_argument("--pose-wait-timeout", type=float, default=20.0,
                    help="Wait timeout for first pose in seconds")
    ap.add_argument("--out-prefix", type=str,
                    default="/home/tyang/sets/plots/drone_two_wp_test",
                    help="Output prefix, without extension")
    ap.add_argument("--pose-topic", type=str, default="",
                    help="Pose topic. Default: /drone_planner/pose_topic or /pose")
    ap.add_argument("--cmd-topic", type=str, default="",
                    help="Command topic. Default: /stanley_controller/cmd_vel_topic or /cmd_vel")
    ap.add_argument("--traj-topic", type=str, default="",
                    help="Trajectory topic. Default: /drone_planner/trajectory_topic or /planner/trajectory")
    ap.add_argument("--use-ros-test-node", action="store_true",
                    help="Start built-in ROS pose test node driven by UCT rollout")
    ap.add_argument("--sim-rate", type=float, default=50.0,
                    help="Pose publish/simulation rate (Hz) for --use-ros-test-node")
    ap.add_argument("--sim-config-name", type=str, default="policy_convergence_drone",
                    help="Config used by ROS test pose node")
    ...
    ap.add_argument("--config-name", type=str, default="policy_convergence_drone",
                    help="Config for obstacle loading in plots")
    args = ap.parse_args()
    ...
    try:
        result = runner.run()
        rospy.loginfo("Mission success: wp0_dist=%.2f, wp1_dist=%.2f",
                      result["wp0_dist"], result["wp1_dist"])
    except Exception as e:
        code = 2
        ...
        runner.shutdown()
        try:
            png_path, gif_path, metrics = save_plots(
                runner.samples, np.array(args.wp0), np.array(args.wp1),
                args.out_prefix, args.config_name)

            # compute trajectory and cmd stats
            traj_ok_arr = np.array([s.get("traj_ok", False) for s in runner.samples], dtype=bool)
            metrics["trajectory_msg_count"] = runner.traj_received_count
            metrics["trajectory_msg_ratio"] = float(np.mean(traj_ok_arr)) if len(traj_ok_arr) > 0 else 0.0

            cmd_v = np.array([s["vx"] for s in runner.samples])
            cmd_vz = np.array([s["vz"] for s in runner.samples])
            cmd_w = np.array([s["yr"] for s in runner.samples])
            cmd_nonzero = (np.abs(cmd_v) > 1e-6) | (np.abs(cmd_vz) > 1e-6) | (np.abs(cmd_w) > 1e-6)
            metrics["cmd_nonzero_ratio"] = float(np.mean(cmd_nonzero)) if len(cmd_nonzero) > 0 else 0.0
